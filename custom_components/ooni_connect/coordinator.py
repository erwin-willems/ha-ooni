...
class OoniConnectCoordinator(DataUpdateCoordinator[Any]):
    ...
    def _handle_bluetooth_update(self, data: Any) -> None:
        ...
        if data is not None:
            ...
            _LOGGER.info(
                "Dir(data): %s", [x for x in dir(data) if not x.startswith("_")]
            )
    ...
    def _on_disconnected(self) -> None:
        ...
        self.client = None
        # Notify entities so connection-related sensors update immediately.
    ...
    async def _async_update_data(self) -> Any:
        ...
        if self.client is None or not self.client.is_connected:
            ...
            if (
                ...
                and (now - self._last_connect_attempt) >= _MIN_RETRY_INTERVAL
            ):
    ...
    async def _connect_in_background(self) -> None:
        ...
        try:
            async with self._lock:
                ...
                if not ble_device:
                    _LOGGER.debug(
                        "Ooni device not reachable at address: %s", self.address
                    )
                    return

                _LOGGER.info(
                    "BLE device found: %s", ble_device.name or ble_device.address
                )
                ...
                try:
                    ...
                    def _notify_data(
                        ...
                        try:
                            ...
                            _LOGGER.error(
                                "Failed to decode: %s with error %s", data, exc
                            )
                            return
                        self._handle_bluetooth_update(packet)

                    await bleak_client.start_notify(
                        MainService.notify.uuid, _notify_data
                    )
                    ...
                    _LOGGER.error(
                        "Background connection failed: %s (%s)",
                        err,
                        type(err).__name__,
                        exc_info=True,
                    )
